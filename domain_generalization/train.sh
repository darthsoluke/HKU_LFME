...
CUDA_VISIBLE_DEVICES=5 python -m domainbed.scripts.train \
  ...
  --test_env 0 --output_dir outputs/PACS_te0 &
#CUDA_VISIBLE_DEVICES=1 python -m domainbed.scripts.train \
#  --data_dir="$DATA" --algorithm "$ALGO" --dataset "$DATASET" \
#  --test_env 1 --output_dir outputs/PACS_te1 &
#CUDA_VISIBLE_DEVICES=2 python -m domainbed.scripts.train \
#  --data_dir="$DATA" --algorithm "$ALGO" --dataset "$DATASET" \
#  --test_env 2 --output_dir outputs/PACS_te2 &
#CUDA_VISIBLE_DEVICES=3 python -m domainbed.scripts.train \
#  --data_dir="$DATA" --algorithm "$ALGO" --dataset "$DATASET" \
#  --test_env 3 --output_dir outputs/PACS_te3 &
wait

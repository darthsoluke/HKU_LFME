// LFME Demo - Internationalization Configuration
...
class I18n {
    ...
    // 获取类别显示名称 - 确保英文版只显示英文，中文版显示中英文对照
    getCategoryName(id) {
        // 直接从对应语言的翻译中获取，确保显示正确的格式
        return this.translations[this.currentLang].categories[id] || `Category ${id}`;
    }
    
    // 获取域类型显示名称 - 确保英文版只显示英文，中文版显示中英文对照
    getDomainName(name) {
        // 直接从对应语言的翻译中获取，确保显示正确的格式
        return this.translations[this.currentLang].domains[name] || name;
    }
    
    // 获取专家名称
    getExpertName(name) {
        return this.translations[this.currentLang].experts[name] || name;
    }
}
